...
pub struct KdeBackend {
    ...
    pub event_queue: wayland_client::EventQueue<KdeState>,
    pub qh: QueueHandle<KdeState>,
    pub state: KdeState,
}
...
impl ProvidesRegistryState for KdeState {
...
impl CompositorHandler for KdeState {
...
impl ShmHandler for KdeState {
    fn shm_state(&mut self) -> &mut Shm {
        &mut self.shm
    }
}

impl LayerShellHandler for KdeState {
    fn closed(
        &mut self,
        _conn: &Connection,
        _qh: &QueueHandle<Self>,
...
impl OutputHandler for KdeState {
...
impl PointerHandler for KdeState {
    fn pointer_frame(
        &mut self,
        _conn: &Connection,
        _qh: &QueueHandle<Self>,
...
impl RelativePointerHandler for KdeState {
...
delegate_compositor!(KdeState);
delegate_shm!(KdeState);
delegate_layer!(KdeState);
delegate_output!(KdeState);
delegate_seat!(KdeState);
delegate_pointer!(KdeState);
delegate_relative_pointer!(KdeState);
delegate_registry!(KdeState);

impl Dispatch<wl_region::WlRegion, GlobalData> for KdeState {
    fn event(
        _state: &mut Self,
...
struct FrameBuffer {
    ...
    // (which outlives KdeWindow since pool stays in KdeWindow).
...
impl DesktopWindow for KdeWindow {
