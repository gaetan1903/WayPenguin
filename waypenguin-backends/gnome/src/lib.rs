...
pub struct GnomeBackend {
    ...
    pub event_queue: wayland_client::EventQueue<GnomeState>,
    pub qh: QueueHandle<GnomeState>,
...
impl ProvidesRegistryState for GnomeState {
...
impl CompositorHandler for GnomeState {
...
impl ShmHandler for GnomeState {
    fn shm_state(&mut self) -> &mut Shm {
        &mut self.shm
    }
}

impl LayerShellHandler for GnomeState {
    fn closed(
        &mut self,
        _conn: &Connection,
        _qh: &QueueHandle<Self>,
...
impl OutputHandler for GnomeState {
...
impl SeatHandler for GnomeState {
...
impl PointerHandler for GnomeState {
    fn pointer_frame(
        &mut self,
        _conn: &Connection,
        _qh: &QueueHandle<Self>,
...
impl RelativePointerHandler for GnomeState {
...
delegate_compositor!(GnomeState);
delegate_shm!(GnomeState);
delegate_layer!(GnomeState);
delegate_output!(GnomeState);
delegate_seat!(GnomeState);
delegate_pointer!(GnomeState);
delegate_relative_pointer!(GnomeState);
delegate_registry!(GnomeState);

impl Dispatch<wl_region::WlRegion, GlobalData> for GnomeState {
    fn event(
        _state: &mut Self,
...
impl DesktopBackend for GnomeBackend {
    ...
    fn create_window(
        ...
        // Align to 64 bytes (SlotPool::new_slot rounds up to 64), then ×2 for two buffers.
        ...
        // Pre-allocate two SHM buffers for double buffering.
        let frames = [
            Self::create_frame(&mut pool, width, height, stride, format)?,
            Self::create_frame(&mut pool, width, height, stride, format)?,
        ];

        Ok(Box::new(GnomeWindow {
            layer_surface,
            _pool: pool,
            width,
            height,
...
pub struct GnomeWindow {
    ...
    width: u32,
    height: u32,
    // Two pre-allocated frame buffers for clean double buffering.
    frames: [FrameBuffer; 2],
    current: usize,
}

/// Wraps a pre-allocated SHM buffer and its writable canvas.
struct FrameBuffer {
    ...
    // (which outlives GnomeWindow since pool stays in GnomeWindow).
...
impl FrameBuffer {
    /// Write ARGB pixels into this buffer's SHM canvas.
...
impl Drop for FrameBuffer {
    fn drop(&mut self) {
        // SAFETY: We manually forgot the Slot, but the wl_buffer must be
        // destroyed. ManuallyDrop::drop runs the inner Buffer's destructor
        // which destroys the wl_buffer proxy.
        unsafe {
...
impl DesktopWindow for GnomeWindow {
    ...
    fn present_pixels(&mut self, pixels: &[u32]) -> Result<(), BackendError> {
        ...
        // Flip to the next pre-allocated buffer — never create a wl_buffer here.
        self.current = (self.current + 1) & 1;
        let fb = &mut self.frames[self.current];

        fb.write_pixels(pixels);

        // Use attach_to which properly activates the buffer for the compositor.
        let surface = self.layer_surface.wl_surface();
        if fb.buffer.attach_to(surface).is_err() {
            // Buffer still active (compositor hasn't released it yet) — attach directly.
